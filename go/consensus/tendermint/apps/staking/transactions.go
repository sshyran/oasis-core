...
func isTransferPermitted(params *staking.ConsensusParameters, from staking.Address) (permitted bool) {
	permitted = true
	if params.DisableTransfers {
		permitted = false
		if params.UndisableTransfersFrom != nil && params.UndisableTransfersFrom[from] {
			permitted = true
		}
	}
	return
...
func (app *stakingApplication) transfer(ctx *api.Context, state *stakingState.MutableState, xfer *staking.Transfer) error {
	...
	fromAddr := staking.NewFromPublicKey(ctx.TxSigner())
...
func (app *stakingApplication) reclaimEscrow(ctx *api.Context, state *stakingState.MutableState, reclaim *staking.ReclaimEscrow) error {
	...
	toAddr := staking.NewFromPublicKey(ctx.TxSigner())
