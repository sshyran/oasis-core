...
func (w *xferWorkload) Init(doc *genesis.Document) error {
	...
	testAccount := doc.Staking.Ledger[staking.NewFromPublicKey(testEntity.ID)]
	if testAccount == nil {
...
func (w *xferWorkload) Start(initialState *genesis.Document, cancelCh <-chan struct{}, errCh chan<- error) (<-chan []BlockTx, error) {
	...
	testAccountID := staking.NewFromPublicKey(testEntity.ID)
	testAccount := initialState.Staking.Ledger[testAccountID]
	...
	for i := 0; i < transferNumAccounts; i++ {
		...
		acc := &xferAccount{
			signer:  accSigner,
			address: staking.NewFromPublicKey(accSigner.Public()),
		}
